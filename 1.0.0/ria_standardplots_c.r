...
plots<-function(title,plottype,plotformat,plotvariable,inputdir,group,count,pngoutput,output_acmo){
    ...
    temp<-NA
    title<-paste(title, " of ",cropname, " in ",df[1,3], sep="")
...
plotformat<-args[3]
plotvariable<-args[4]
inputdir<-args[5]#"/Users/weixiong/Development/face-it/RIA/ria_standardplots/1.0.5_either input zip or csv"
group<-args[6]
count<-args[7]
pngoutput<-args[8]
output_acmo<-args[9]
